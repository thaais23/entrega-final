...
# Mini juego reorganizado y bonito
    ...
    if st.session_state.ronda <= 3 and not st.session_state.juego_terminado:
        ...
        with col1:
            if st.button("✅ VERDADERO", key=f"v_{st.session_state.ronda}"):
                if st.session_state.episodios_mostrados == st.session_state.drama_actual['number_of_episodes']:
                    ...
                    st.error(f"Incorrecto. Tiene {st.session_state.drama_actual['number_of_episodes']} episodios.")
                st.session_state.ronda += 1
                st.session_state.pop("drama_actual")
        ...
        if st.button("🔄 Volver a jugar"):
            ...
            st.session_state.ronda = 1
            st.session_state.juego_terminado = False
            if "drama_actual" in st.session_state:
                st.session_state.pop("drama_actual")
            st.experimental_rerun()
