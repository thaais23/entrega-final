...
# MINI JUEGO
    ...
    st.info("🖱️ Recuerda: a veces se necesita hacer doble clic para avanzar a la siguiente ronda.")
    ...
    if st.session_state.estado == "respuesta":
        ...
        with col1:
            if st.button("✔️ Verdadero"):
                ...
                st.session_state.estado = "veredicto"
        with col2:
            if st.button("❌ Falso"):
                st.session_state.respuesta = "Falso"
                st.session_state.estado = "veredicto"

    if st.session_state.estado == "veredicto":
        drama = st.session_state.drama
        correcta = (
